...
class SecondClaimCalculator {
  ...
  public calculateInstallments(
    ...
    const prepaymentMap = new Map(prepayments.map(p => [SecondClaimCalculator.formatDateOnly(new Date(p.date)), p.amount]));
    const disbursementMap = new Map(disbursements.map(d => [SecondClaimCalculator.formatDateOnly(new Date(d.date)), d.amount]));
    const holidayMonthsSet = new Set(holidayMonths.map(h => SecondClaimCalculator.formatDateOnly(new Date(h.date))));
    ...
    for (let i = 0; i < loanTerms; i++) {
      ...
      const wiborRate = this.getWiborRate(currentDate, type);
      ...
      if (i >= gracePeriodMonths) {
        if (installmentType === 'malejące') {
          principalPayment = SecondClaimCalculator.calculatePMT(currentRate / 100 / 12, loanTerms - i, remainingAmount) - interestPayment;
        } else {
