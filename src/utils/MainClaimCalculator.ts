...
class MainClaimCalculator {
  ...
  public calculateInstallments(
    ...
    const prepaymentMap = new Map(prepayments.map(p => [MainClaimCalculator.formatDateOnly(new Date(p.date)), p.amount]));
    const disbursementMap = new Map(disbursements.map(d => [MainClaimCalculator.formatDateOnly(new Date(d.date)), d.amount]));
    const holidayMonthsSet = new Set(holidayMonths.map(h => MainClaimCalculator.formatDateOnly(new Date(h.date))));
    ...
    for (let i = 0; i < loanTerms; i++) {
      ...
      if (i >= gracePeriodMonths) {
        if (installmentType === 'malejące') {
          principalPayment = MainClaimCalculator.calculatePMT(currentRate / 100 / 12, loanTerms - i, remainingAmount) - interestPayment;
        } else {
